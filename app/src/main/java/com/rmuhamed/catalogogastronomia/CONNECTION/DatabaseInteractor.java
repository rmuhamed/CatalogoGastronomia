...
/**
 * DatabaseInteractor encapsulate the db create, updates and backup..
 * @author maltamira
 *
 */
public class DatabaseInteractor extends SQLiteOpenHelper {

	private static final String DATABASE_NAME = "catalogo-database.db";
	...
	public void onCreate(SQLiteDatabase database) {
		...
		if(DatabaseStructure.getInstance().getUpdates()!=null && DatabaseStructure.getInstance().getUpdates().size()>0){
			for(int version = database.getVersion() + 1; version <= databaseVersion; version++){
				...
				try {
					if(DatabaseStructure.getInstance().getUpdates().get(Integer.valueOf(version))!=null){
	...
	public boolean removeDatabaseBackup(){
		boolean succed = false;
		try {
			SQLiteDatabase database = this.getWritableDatabase();
			String databasePath = database.getPath();
	...
	public void addExecutedUpdate(int updateNumber){
